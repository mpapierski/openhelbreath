...
void LoadingScene::OnLoop()
{
	...
	{
		case 0:
			Game::GetInstance().Sprites[SPRID_MAINMENU].LoadFromFile(
					"New-Dialog.pak", 1);
			Game::GetInstance().Sprites[SPRID_EXIT].LoadFromFile(
					"New-Dialog.pak", 2);
			Game::GetInstance().Sprites[SPRID_LOGIN].LoadFromFile(
					"LoginDialog.pak", 0);
			Game::GetInstance().Sprites[SPRID_GAMEDIALOG_3].LoadFromFile(
					"GameDialog.pak", 3);
...
void LoadingScene::Draw(SDL_Surface *Dest)
{
	...
	Sprite::Draw(Dest, Game::GetInstance().Sprites[SPRID_LOADING], 1, 1,
			SPRID_LOADING_BACKGROUND);
	Sprite::Draw(Dest, Game::GetInstance().Sprites[SPRID_LOADING], 473, 443,
			Percent, 17, SPRID_LOADING_PROGRESSBAR);
}
